...
local function logics()
    ...
    local is_logic_allowed = my_utility.is_spell_allowed(
        ...
        my_utility.abilities.spell_id_counterattack);
    ...
    local is_buff_active = my_utility.is_buff_active(my_utility.abilities.spell_id_counterattack,
        my_utility.abilities.buff_id_counterattack);
    ...
    if health_percent <= menu_min_percentage then
        if cast_spell.self(my_utility.abilities.spell_id_counterattack, 0.000) then
            ...
            console.print("Cast Counterattack - Defensive - " .. health_percent)
            return true;
        end
    end;
    ...
    if use_offensively then
        ...
        then
            if cast_spell.self(my_utility.abilities.spell_id_counterattack, 0.000) then
                local current_time = get_time_since_inject();
                next_time_allowed_cast = current_time + my_utility.spell_delays.instant_cast;
                console.print("Cast Counterattack - Offensive - " .. filter_mode)
                return true;
            end
        end
    end
